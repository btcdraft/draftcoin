...
{
    // DraftCoin: check prefix
    if(uri.scheme() != QString("DraftCoin"))
        return false;
...
{
    // Convert DraftCoin:// to DraftCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DraftCoin://"))
    {
        uri.replace(0, 12, "DraftCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DraftCoin.lnk";
}
...
{
    return GetAutostartDir() / "DraftCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DraftCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DraftCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DraftCoin-Qt"));
